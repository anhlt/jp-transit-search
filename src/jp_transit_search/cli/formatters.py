...
def format_route_table(route: Route, verbose=False) -> str:
    ...
    table = Table(title=f"Route: {route.from_station} → {route.to_station}", show_header=True, header_style="bold magenta")
    ...
    if verbose or route.transfers:
        ...
        if route.transfers:
            transfer_table = Table(title="Transfer Details", show_header=True, header_style="bold blue")
...
[bold]Cost:[/bold] {transfer.cost_yen}円"""
            ...
            with console.capture() as capture:
                console.print(Panel(transfer_text, title=f"Segment {i}", border_style="green"))
            output += capture.get()
