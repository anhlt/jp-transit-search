...
from jp_transit_search.core.models import Route, Transfer, Station
...
class TestTransitMCPServer:
    ...
    def server(self):
        ...
        with patch('jp_transit_search.mcp.server.StationSearcher'):
            with patch('jp_transit_search.mcp.server.StationCrawler'):
                with patch('jp_transit_search.mcp.server.YahooTransitScraper'):
                    ...
                    server.station_crawler = MagicMock()
    ...
    async def test_list_tools(self, server):
        ...
        handler_func = getattr(server.server, 'list_tools')
    ...
    async def test_search_route_validation_error(self, server):
        ...
        with patch.object(server.scraper, 'search_route', side_effect=ValidationError("Empty station name")):
            result = await server._search_route({
                "from_station": "",
                "to_station": "Osaka"
            })
    ...
    async def test_search_route_not_found(self, server):
        ...
        with patch.object(server.scraper, 'search_route', side_effect=RouteNotFoundError("No route found")):
            result = await server._search_route({
                "from_station": "NonExistent",
                "to_station": "AlsoNonExistent"
            })
    ...
    async def test_search_stations_success(self, server, sample_stations):
        ...
        with patch.object(server.station_searcher, 'search_stations', return_value=sample_stations):
    ...
    @pytest.mark.asyncio
    async def test_crawl_stations_default_params(self, server, sample_stations):
        """Test station crawling with default parameters."""
        with patch.object(server.station_crawler, 'crawl_stations', return_value=sample_stations) as mock_crawl:
            await server._crawl_stations({})
            
            # Check default parameters are used
            mock_crawl.assert_called_once_with(prefectures=None, limit=1000)
    
    @pytest.mark.asyncio
    async def test_list_station_database_success(self, server, sample_stations):
        """Test successful station database listing."""
        with patch.object(server.station_searcher, 'list_stations', return_value=sample_stations):
    ...
    async def test_list_station_database_default_params(self, server, sample_stations):
        ...
        with patch.object(server.station_searcher, 'list_stations', return_value=sample_stations) as mock_list:
    ...
    async def test_handle_call_tool_exception(self, server):
        ...
        with patch.object(server.scraper, 'search_route', side_effect=ValidationError("Test validation error")):
            result = await server._search_route({"from_station": "A", "to_station": "B"})
    ...
    def test_server_initialization(self, server):
        ...
        assert server.station_crawler is not None
        ...
        assert hasattr(server.server, 'list_tools')
        assert callable(getattr(server.server, 'list_tools'))


@pytest.mark.asyncio
async def test_main_function():
    """Test the main function runs without errors."""
    with patch('jp_transit_search.mcp.server.TransitMCPServer') as mock_server_class:
        mock_server = MagicMock()
        mock_server.server.stdio_transport.return_value.__aenter__ = AsyncMock(return_value=('read', 'write'))
        mock_server.server.stdio_transport.return_value.__aexit__ = AsyncMock(return_value=None)
        mock_server.server.run = AsyncMock()
        mock_server.server.get_capabilities = MagicMock(return_value={'tools': {}})
        mock_server_class.return_value = mock_server
        
        # Import and test main
        from jp_transit_search.mcp.server import main
